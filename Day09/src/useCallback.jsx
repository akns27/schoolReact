...
export default function UserCallback() {
  ...
  const onToggle = useCallback((id, user) => {
    //2. 1)이름 클릭하면 selected속성 반전 2) input에 현재값 반영하기
    setUsers(
      users.map((user) =>
        user.id === id
          ? {
  ...
  const onModify = (user, id) => {
    //3.   1)수정 버튼 누르면 input의 값으로 user수정하기 2) input clear
  ...
  const countSelected = (users) => users.filter((user)=> user.selected == true).length;//선언만하고

  //useMemo(함수, [의존변수])
  const sCount = useMemo(()=>countSelected(users), [users]);//값을 넣는건 여기


  return (
    <div>
      {" "}
      {/* 4. input의 name, email로 user객체 요소 렌더링, 컴포넌트에 수정,등록, 삭제 함수 전달 */}
      <UserInput
        name={name}
        email={email}
        onChange={onChange}
        onCreate={onCreate}
      />
      {/* 5. map함수로 출력하는 UserList 컴포넌트를 작성하고, onToggle,onModify함수도 전달하시오. */}
      <UserList
        users={users}
        onRemove={onRemove}
        onToggle={onToggle}
